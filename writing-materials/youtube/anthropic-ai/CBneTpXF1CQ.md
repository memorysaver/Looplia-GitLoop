...
published: 20251030
downloaded_at: 2025-11-28T16:35:40.441659+00:00
---
...
Cloud Code on the web at claw.ai/code.
...
its own isolated sandbox with full
[music] git access and we can watch the
...
certain tasks like [music] computer use.
Now the benchmarks are impressive. 73.3%
...
developers use Haiku [music] 4.5. And
the big pattern that we're seeing from
our community is that Sonnet 4.5 does
the highlevel problem decomposition and
planning and then Haiku 4.5 is used to
actually implement and write [music] the
code. Now this multi- aent approach is
...
that have been changed. [music] I can go
into VS Code and see all of the changes
that were made to the service signup,
the signup.touchcript file, and much
more. And I can continue right from
where I left off on my mobile phone or
the browser. Now, I want to show you one
final feature that we recently added to
Clot Code.
I gave it this prompt earlier
talking about adding [music] caching to
my application, but I gave it a very
broad prompt saying, "Hey, I would like
to add caching to my application." And
what Claude Code did is it's [music]
going to ask me a number of follow-up
questions to make sure that it
implements it correctly. So, it's going
to ask the caching level here. Uh,
should it be database query caching, API
response caching? And I can choose any
of these, go to the next section and
[music] figure out what my goals are. Is
it to improve performance, reduce
database load, the target area, is it
for the workout data, for the user data,
analytics, choose the technology,
...
at home [music] and on the go. Haiku 4.5
for lightning fast coding at a fraction
of the cost [music] and smarter local
workflows with multi-question support.
Cloud Code on the web is available now
for pro and max users at cloud.ai/code.
